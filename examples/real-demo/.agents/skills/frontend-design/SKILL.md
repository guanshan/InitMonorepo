...
## Monorepo Constraints (Read First)

This skill is loaded inside this monorepo. When the user's request targets anything under `apps/web/**` or `packages/ui/**`, the repo's real stack **overrides** the generic aesthetic guidance below:
...
Remember: bold, distinctive design is possible without sacrificing the repo rules above. For in-repo surfaces, creativity must live inside CSS Modules + semantic tokens + i18n-ready copy. For standalone artifacts and prototypes, go as far as the aesthetic vision requires.
