...
This skill enables retrieval of current documentation for software libraries and components by querying the Context7 **HTTP API via `curl`**. Use it instead of relying on potentially outdated training data.

> This repo does **not** ship a Context7 MCP server configuration (no `.mcp.json` at the time of writing). If a Context7 MCP is added later, prefer the MCP tool over raw `curl` for lower-latency and structured results, and update this skill accordingly.
